...
class NFSServer:
	...
	def O_Compound(self, ncl, cmp4args):
		try:
			...
			self.turn_around()
			cmp4res = COMPOUND4res(self, NFS4ERR_NOTSUPP, '', [])
			cmp4res.pack()
			return
		...
		if cmp4args.minorversion <> 0:
			...
			self.turn_around()
			cmp4res = COMPOUND4res(self, NFS4ERR_MINOR_VERS_MISMATCH, '', [])
			cmp4res.pack()
			return
	...
	#FIXME:  Needs real implementation, on server and client side.
        def O_Lock(self, ncl, op):
                print "SERVER LOCK"
                print "  CURRENT FILEHANDLE %s" % self.curr_fh.ref
                if 1:
                        # Write to lock attribute in filehandle?
                        lock_stateid = stateid4(ncl,
                        ...
                        lock_denied = "something else"
                        l4res = LOCK4res(ncl, NFS4ERR_DENIED, lock_denied)
                        argop = nfs_resop4(ncl, resop=OP_LOCK, oplock=l4res)
